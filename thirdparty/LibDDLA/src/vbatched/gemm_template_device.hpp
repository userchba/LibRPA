...
static __device__
    DdlaFloatComplex
    atomicAdd(DdlaFloatComplex *address, DdlaFloatComplex val)
{
    float* components = reinterpret_cast<float*>(address);
    float re = atomicAdd(components, val.real());
    float im = atomicAdd(components + 1, val.imag());
    return DDLA_C_MAKE(re, im);
}

static __device__
    DdlaDoubleComplex
    atomicAdd(DdlaDoubleComplex *address, DdlaDoubleComplex val)
{
    double* components = reinterpret_cast<double*>(address);
    double re = atomicAdd(components, val.real());
    double im = atomicAdd(components + 1, val.imag());
    return DDLA_Z_MAKE(re, im);
}
...
void gemm_template_device_nn(
    ...
    T* sC, int sldc )
{
#if defined(DDLA_USE_HIP) || defined(DDLA_USE_CUDA)
    int idx = threadIdx.x;  // thread's m dimension
    int idy = threadIdx.y;  // thread's n dimension
    ...
    } else if( beta == make_FloatingPoint(1.0,0.0) ) {
        #pragma unroll
        for (n = 0; n < THR_N; n++) {
            int coord_dCn = bly*BLK_N + n*DIM_Y + idy;
            #pragma unroll
...
#endif /* DDLA_USE_HIP || DDLA_USE_CUDA */
}
